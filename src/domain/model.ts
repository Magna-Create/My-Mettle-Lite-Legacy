...
export type LoadRelationship = 'external' | 'assistance' | 'bodyweight' | 'bodyweight_plus_external' | 'none';
...
export interface BodyMeasurement {
  ...
  source: 'manual' | 'health_connect' | 'samsung_health'; sourceRecordId?: string; schemaVersion: number;
}

export type VibrationStrength = 'low' | 'medium' | 'strong' | 'very_strong';
export interface RestTimerSettings {
  autoStart: boolean; vibrationEnabled: boolean; vibrationStrength: VibrationStrength;
  chimeEnabled: boolean; backgroundNotificationEnabled?: boolean;
}
export interface AppSettings { restTimer: RestTimerSettings; schemaVersion: number; }
...
export type HealthObservationType = 'exercise_session' | 'heart_rate' | 'resting_heart_rate' | 'heart_rate_variability' | 'sleep' | 'weight' | 'body_composition';
export interface HealthObservation {
  id: Id; type: HealthObservationType; startTime: string; endTime?: string; value?: number; unit?: string;
  provider: Exclude<HealthProviderKind, 'none'>; sourceRecordId: string; sourceDevice?: string;
  payload?: Record<string, unknown>; importedAt: string; schemaVersion: number;
}
export interface HealthIntegrationState {
  provider: HealthProviderKind; permissionState: HealthPermissionState; lastSyncedAt?: string; lastError?: string; schemaVersion: number;
}

export interface UserProfile {
  id: Id; displayName: string; units: LoadUnit; dietaryPreference: 'vegetarian'; cycleStartDay: number;
...
export interface RoutineSlot {
  id: Id; exerciseId: Id; position: number; importance: Importance; plannedLoad: number;
  prescriptions: Record<Mode, ModePrescription>; lockedToDay: boolean;
}
export interface RoutineDay { symbol: DaySymbol; slots: RoutineSlot[]; }
export interface RoutineVersion {
  id: Id; version: number; parentId?: Id; createdAt: string; effectiveAt: string;
  source: 'seed' | 'manual_edit' | 'experiment_promotion'; changeReason: string; days: RoutineDay[]; schemaVersion: number;
}

export type SetKind = 'prescribed' | 'additional' | 'warm_up';
export interface SetRecord {
  id: Id; setIndex: number; load: number | null; reps: number | null; durationSeconds: number | null;
  distanceMetres: number | null; unit: LoadUnit; completedAt?: string | undefined; note?: string;
...
export interface Experiment {
  id: Id; exerciseId: Id; routineSlotId: Id; exerciseName: string; hypothesis: string; baselineLoad: number;
  proposedLoad: number; targetRepMin: number; status: ExperimentStatus; createdAt: string; activatedAt?: string;
  testedSessionId?: Id; evidenceSummary?: string; adoptedRoutineVersionId?: Id; schemaVersion: number;
}

export interface TrainingCycle {
  id: Id; startedAt: string; endedAt?: string; status: 'active' | 'closed'; completedCoreDays: CoreDay[];
  andCompleted: boolean; schemaVersion: number;
}

export interface AppDatabase {
  profile: UserProfile; settings: AppSettings; bodyMeasurements: BodyMeasurement[]; exercises: Exercise[];
  routineVersions: RoutineVersion[]; currentRoutineVersionId: Id; sessions: Session[]; cycles: TrainingCycle[];
  currentCycleId: Id; activeSessionId: Id | null; experiments: Experiment[]; healthObservations: HealthObservation[];
  healthIntegration: HealthIntegrationState; createdAt: string; updatedAt: string; schemaVersion: number;
}
