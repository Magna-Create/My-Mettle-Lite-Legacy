...
export function ProfileSheetV2(props: Props) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const input: { recordedAt: string; weightKg?: number; heightCm?: number } = {
      recordedAt: new Date(`${String(form.get('date'))}T12:00:00`).toISOString(),
    };
    ...
    <>
      <div className="modal-backdrop settings-backdrop" onMouseDown={onClose}>
        <aside className="settings-sheet profile-sheet" onMouseDown={(event) => event.stopPropagation()}>
          ...
          <button className="profile-history-button" type="button" onClick={() => setHistoryOpen(true)}><span><strong>Session history</strong><small>Review, amend, exclude or discard individual sessions.</small></span><i>›</i></button>
          <section className="measurement-section">
            ...
            {adding && <form className="measurement-form" onSubmit={submit}>
      ...
      {historyOpen && <SessionHistoryOverlay database={database} onClose={() => setHistoryOpen(false)} onAmendSet={props.onAmendSet} onAddSet={props.onAddSet} onRemoveSet={props.onRemoveSet} onSetExcluded={props.onSetExcluded} onDiscard={props.onDiscardSession} onRestore={props.onRestoreSession} />}
    </>
  );
}