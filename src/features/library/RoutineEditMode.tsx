...
export const ROUTINE_EDIT_STORAGE_KEY = 'my-mettle:routine-edit-draft:v1';
...
const dayLabels: Record<DaySymbol, string> = {
  'ψ': 'core day',
  'φ': 'core day',
  'π': 'core day',
...
function sameStructure(left: RoutineEditDraft, right: RoutineEditDraft) {
  return left.days.every((day, dayIndex) => {
...
function hapticDescription(database: AppDatabase, slot: RoutineSlot, targetDay: DaySymbol) {
...
function targetIndexWithoutMovingSlot(
  draft: RoutineEditDraft,
  movingSlotId: string,
  targetDay: DaySymbol,
...
function autoScroll(clientY: number) {
  ...
  if (clientY < edge) {
    ...
    window.scrollBy(0, -Math.ceil(18 * strength));
...
export function RoutineEditMode({ database, initialDraft, externalDiscardToken, onStateChange, onCommit, onExit }: Props) {
  const [draft, setDraft] = useState<RoutineEditDraft>(initialDraft);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(ROUTINE_EDIT_STORAGE_KEY, JSON.stringify(draft));
  ...
  useEffect(() => {
    ...
    onExit();
  }, [externalDiscardToken, onExit, onStateChange]);

  const exerciseById = useMemo(
    () => new Map(database.exercises.map((exercise) => [exercise.id, exercise])),
    [database.exercises],
  );
  ...
  function applyDraftChange(next: RoutineEditDraft, message: string) {
    if (sameStructure(draft, next)) return;
    pushUndo(draft, message);
    setDraft(next);
  ...
  function beginDrag(event: ReactPointerEvent<HTMLButtonElement>, slotId: string) {
    ...
    const location = locateDraftSlot(draft, slotId);
    ...
    setDrag({
      ...
      width: rect.width,
      height: rect.height,
      originDraft: structuredClone(draft),
      originDay: location.day.symbol,
  ...
  function updateDrag(event: ReactPointerEvent<HTMLButtonElement>) {
    ...
    const rail = elements.find((element) => element.dataset.editRailDay) as HTMLElement | undefined;
    const lane = elements.find((element) => element.dataset.editDay) as HTMLElement | undefined;
    const targetCard = elements.find((element) => element.dataset.editSlot && element.dataset.editSlot !== drag.slotId) as HTMLElement | undefined;
    ...
    let targetSlotId: string | null = targetCard?.dataset.editSlot ?? null;
    let placeAfter = false;
    if (targetCard) {
      const targetRect = targetCard.getBoundingClientRect();
      placeAfter = event.clientY > targetRect.top + targetRect.height / 2;
    }
    if (rail) targetSlotId = null;
    const targetIndex = targetIndexWithoutMovingSlot(draft, drag.slotId, targetDay, targetSlotId, placeAfter);
    const key = `${targetDay}:${targetIndex}`;
    if (lastTargetRef.current === key) return;

    const previousDay = locateDraftSlot(draft, drag.slotId).day.symbol;
    const next = moveRoutineDraftSlot(draft, drag.slotId, targetDay, targetIndex);
    if (sameStructure(draft, next)) return;
  ...
  function finishDrag(event: ReactPointerEvent<HTMLButtonElement>) {
    if (!drag || event.pointerId !== drag.pointerId) return;
    const location = locateDraftSlot(draft, drag.slotId);
    const changed = !sameStructure(drag.originDraft, draft);
    if (changed) {
      const message = hapticDescription(database, location.slot, location.day.symbol);
      pushUndo(drag.originDraft, message);
      routineEditHaptic('place');
    }
    setDrag(null);
  ...
  function moveFromMenu(slotId: string, targetDay: DaySymbol, targetIndex: number) {
    const slot = locateDraftSlot(draft, slotId).slot;
    const next = moveRoutineDraftSlot(draft, slotId, targetDay, targetIndex);
    applyDraftChange(next, hapticDescription(database, slot, targetDay));
    setMenuSlotId(null);
  }
  ...
  function removeFromDraft(slotId: string) {
    ...
    applyDraftChange(removeRoutineDraftSlot(draft, slotId), `Removed ${exercise?.name ?? 'exercise'} from the draft`);
    setMenuSlotId(null);
  }
  ...
  return <main className="page routine-edit-page">
    ...
    {drag && <nav className="routine-destination-rail" aria-label="Move exercise to day">
      <span>Move to</span>
      {days.map((day) => <button key={day} type="button" data-edit-rail-day={day}>{day}</button>)}
    </nav>}
      ...
      {draft.days.map((day) => <article className="routine-edit-lane" key={day.symbol} data-edit-day={day.symbol}>
        ...
        <div className={`routine-edit-slot-list ${day.slots.length === 0 ? 'is-empty' : ''}`}>
          ...
          {day.slots.map((slot) => {
            ...
            return <article
              className={`routine-edit-card ${isDragging ? 'is-dragging-source' : ''}`}
              key={slot.id}
              data-edit-card
              data-edit-slot={slot.id}
              data-edit-card-day={day.symbol}
            >
              ...
              <button
                ...
                onPointerDown={(event) => beginDrag(event, slot.id)}
    ...
    {drag && draggedSlot && draggedExercise && <div className="routine-drag-ghost" style={{
      '--drag-x': `${drag.clientX - drag.offsetX}px`,
      '--drag-y': `${drag.clientY - drag.offsetY}px`,
      '--drag-width': `${drag.width}px`,
      '--drag-height': `${drag.height}px`,
    } as CSSProperties}><span className="importance-dot" data-importance={draggedSlot.importance} /><div><strong>{draggedExercise.name}</strong><small>{draggedSlot.importance}</small></div><span>≡</span></div>}
    ...
    {discardPrompt && <div className="modal-backdrop routine-discard-backdrop"><section className="modal routine-discard-dialog"><p className="eyebrow">Unfinished edit</p><h2>Discard routine changes?</h2><p>The current routine remains untouched.</p><footer><button className="secondary-action" type="button" onClick={() => setDiscardPrompt(false)}>Keep editing</button><button className="danger-action" type="button" onClick={() => { localStorage.removeItem(ROUTINE_EDIT_STORAGE_KEY); onStateChange({ editing: false, dirty: false }); onExit(); }}>Discard</button></footer></section></div>}
  </main>;
}
