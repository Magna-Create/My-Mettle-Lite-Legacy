...
import type { AppDatabase, DaySymbol, EntryBasis, ExerciseTrackingProfile, Importance, Mode, ModePrescription } from '../../domain/model';
import {
  entryBasisLabel,
...
export function LibraryPageV2(props: Props) {
  ...
  function closeWizard() {
    setOpen(false); setStep(0); setDraft(emptyDraft); setError(null);
  }
  ...
  async function saveNew() {
    ...
    try {
      ...
      await props.onAddExercise({
        name: draft.name.trim(), day: draft.day, importance: draft.importance, tracking,
        startingValue: presentation.requiresStartingValue ? draft.startingValue : 0,
        targetValue: draft.targetValue, progressionStep: draft.progressionStep,
      });
      closeWizard();
    } catch (reason) {
  ...
  function updatePrescription(mode: Mode, key: keyof ModePrescription, value: number | boolean) {
    setManageDraft((current) => current ? {
      ...current,
      prescriptions: { ...current.prescriptions, [mode]: { ...current.prescriptions[mode], [key]: value } },
    } : null);
  }

  return (
    <main className="page library-page">
      <section className="page-heading library-heading"><div><p className="eyebrow">Library · routine v{routine.version}</p><h1>How the training system is constructed.</h1></div><button className="primary-action compact" onClick={() => setOpen(true)}>Add exercise</button></section>
