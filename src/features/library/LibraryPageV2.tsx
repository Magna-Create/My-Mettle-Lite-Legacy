...
export function LibraryPageV2(props: Props) {
  ...
  const selected = routine?.days
    .flatMap((day) => day.slots.map((slot) => ({ day, slot })))
    .find((item) => item.slot.id === manageSlotId);
  ...
  function choosePreset(preset: TrackingPreset) {
    setDraft((current) => ({
      ...current,
      preset,
      entryBasis: ['assisted_bodyweight', 'bodyweight', 'reps_only', 'duration', 'distance'].includes(preset) ? 'total' : current.entryBasis,
      ...defaultsForPreset(preset),
    }));
  }

  function moveStep(direction: -1 | 1) {
    if (direction === 1 && step === 0 && !draft.name.trim()) {
      setError('Give the movement a name first.');
      return;
    }
    setError(null);
  ...
  async function saveNew() {
    if (draft.targetValue <= 0 || draft.progressionStep <= 0) {
      setError('Check the training dose.');
      return;
    }
    try {
      setSaving(true);
      setError(null);
      await props.onAddExercise({
    ...
    try {
      ...
      await props.onUpdateExercise(selectedExercise.id, {
        ...
        memory: {
          ...
          videoReferenceUrl: manageDraft.videoReferenceUrl,
          machineSettings: manageDraft.machineSettings,
          substitutions: splitLines(manageDraft.substitutions),
        },
      });
      await props.onUpdateSlot(selected.slot.id, {
        importance: manageDraft.importance,
        plannedLoad: manageDraft.plannedLoad,
        prescriptions: manageDraft.prescriptions,
      });
  ...
  function updatePrescription(mode: Mode, key: keyof ModePrescription, value: number | boolean) {
    setManageDraft((current) => current ? {
      ...current,
      prescriptions: {
        ...current.prescriptions,
        [mode]: { ...current.prescriptions[mode], [key]: value },
      },
    } : null);
  }
    ...
    <section className="routine-board">
      {routine.days.map((day) => <article className="routine-column" key={day.symbol}>
        ...
        {day.slots.map((slot, index) => {
          ...
          return <div className="routine-slot managed-slot" key={slot.id}>
            <span className="importance-dot" data-importance={slot.importance} />
            <button className="routine-slot-content" type="button" onClick={() => setManageSlotId(slot.id)}><strong>{exercise.name}</strong><small>{slot.importance} · {view.requiresStartingValue ? `${view.valueLabel} ${slot.plannedLoad} ${view.valueSuffix}` : TRACKING_PRESET_LABELS[presetFromTracking(exercise.tracking)]} · A {slot.prescriptions.A.sets}×{slot.prescriptions.A.repMin}–{slot.prescriptions.A.repMax}</small></button>
            <div className="routine-slot-controls">
              <button type="button" disabled={index === 0} onClick={() => { void props.onReorderSlot(slot.id, -1); }} aria-label={`Move ${exercise.name} up`}>↑</button>
              <button type="button" disabled={index === day.slots.length - 1} onClick={() => { void props.onReorderSlot(slot.id, 1); }} aria-label={`Move ${exercise.name} down`}>↓</button>
              <button type="button" onClick={() => setManageSlotId(slot.id)} aria-label={`Manage ${exercise.name}`}>•••</button>
            </div>
          </div>;
        })}
      </article>)}
    ...
    {selected && selectedExercise && manageDraft && <div className="modal-backdrop manage-exercise-backdrop" onMouseDown={() => setManageSlotId(null)}><section className="manage-exercise-surface" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="manage-exercise-scroll">
        ...
        <section className="memory-editor">
          <p className="eyebrow">Exercise memory</p>
          <label>Target muscles<textarea value={manageDraft.targetMuscles} onChange={(event) => setManageDraft({ ...manageDraft, targetMuscles: event.target.value })} placeholder="One per line" /></label>
          <label>Key cues<textarea value={manageDraft.cues} onChange={(event) => setManageDraft({ ...manageDraft, cues: event.target.value })} placeholder="One per line" /></label>
          <label>Common mistakes<textarea value={manageDraft.commonMistakes} onChange={(event) => setManageDraft({ ...manageDraft, commonMistakes: event.target.value })} /></label>
          <label>Setup notes<textarea value={manageDraft.setupNotes} onChange={(event) => setManageDraft({ ...manageDraft, setupNotes: event.target.value })} /></label>
          <label>Video reference<input type="url" inputMode="url" value={manageDraft.videoReferenceUrl} onChange={(event) => setManageDraft({ ...manageDraft, videoReferenceUrl: event.target.value })} placeholder="https://youtube.com/…" /></label>
          <label>Machine / equipment settings<textarea value={manageDraft.machineSettings} onChange={(event) => setManageDraft({ ...manageDraft, machineSettings: event.target.value })} /></label>
          <label>Substitutions<textarea value={manageDraft.substitutions} onChange={(event) => setManageDraft({ ...manageDraft, substitutions: event.target.value })} placeholder="One per line" /></label>
        </section>
        {error && <p className="form-error">{error}</p>}
      </div>
