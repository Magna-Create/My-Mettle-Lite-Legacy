...
interface Props { database: AppDatabase; exercise: SessionExercise | null; onClose: () => void; }
...
export function ExerciseDetailsOverlay({ database, exercise, onClose }: Props) {
  ...
  return <div className="modal-backdrop details-backdrop" onMouseDown={onClose}>
    <section className="exercise-details" role="dialog" aria-modal="true" aria-labelledby="exercise-details-title" onMouseDown={(event) => event.stopPropagation()}>
      <header><div><p className="eyebrow">Additional details</p><h2 id="exercise-details-title">{exercise.exerciseNameSnapshot}</h2></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close exercise details">×</button></header>
      ...
      <section className="details-section"><p className="eyebrow">Setup</p><p>{memory.setupNotes || memory.cues[0] || 'No setup notes saved yet.'}</p></section>
      ...
      <section className="details-section"><p className="eyebrow">Personal notes</p><p>{memory.personalNotes || 'No personal notes saved yet.'}</p></section>
