...
import { calculateSessionCelebration, type SessionCelebrationSummary } from '../../domain/rules/sessionCelebration';
...
type SetPatch = Partial<Pick<SetRecord, 'load' | 'reps' | 'durationSeconds' | 'distanceMetres' | 'note'>>;
...
function targetSuffix(metric: string) {
  return metric === 'duration' ? 'sec' : metric === 'distance' ? 'm' : 'reps';
}
...
export function TrainPage({ database, onUpdateSet, onAddSet, onRemoveSet, onUpdateExercise, onSaveReflection, onCompleteExercise, onCompleteSession, onGoBrief, onProgressState, onStartRest }: Props) {
  ...
  const [detailsInitialSection, setDetailsInitialSection] = useState<'info' | 'setup'>('info');
  ...
  const [celebration, setCelebration] = useState<SessionCelebrationSummary | null>(null);
  const progressAnchorRef = useRef<HTMLDivElement | null>(null);
  const pendingRestSetIdsRef = useRef(new Set<string>());
  ...
  useEffect(() => {
    if (!session) {
      ...
      setReflectionExerciseId(null);
      setUndoRecord(null);
      setCelebration(null);
      pendingRestSetIdsRef.current.clear();
      return;
    }
  ...
  async function updateSetWithUndo(exerciseId: string, set: SetRecord, patch: SetPatch) {
    ...
    if (!wasComplete && willComplete) pendingRestSetIdsRef.current.add(set.id);
    if (!willComplete) pendingRestSetIdsRef.current.delete(set.id);
  ...
  function commitSetField(exerciseId: string, set: SetRecord, patch: SetPatch) {
    const exercise = liveSession.exercises.find((candidate) => candidate.id === exerciseId);
    if (!exercise) return;
    const willComplete = isSetComplete({ ...set, ...patch }, exercise.trackingSnapshot);
    if (!willComplete) {
      pendingRestSetIdsRef.current.delete(set.id);
      return;
    }
    if (!pendingRestSetIdsRef.current.delete(set.id)) return;
  ...
  async function undoLastSetEdit() {
    ...
    setUndoRecord(null);
    pendingRestSetIdsRef.current.delete(record.setId);
  ...
  function openDetails(exerciseId: string, section: 'info' | 'setup') {
  ...
  return <main className="page train-page">
    <header className="session-header" ref={progressAnchorRef}>
      <div><p className="eyebrow">Active session</p><h1>{liveSession.day} · {MODE_PRESENTATION[liveSession.mode].name}</h1></div>
      <div className="session-progress"><span>{completed}/{liveSession.exercises.length}</span><div><i style={{ width: `${progress * 100}%` }} /></div></div>
    </header>
    ...
    <section className="exercise-stack">{liveSession.exercises.map((exercise, index) => {
      ...
      const record = database.exercises.find((item) => item.id === exercise.exerciseId);
      const memory = normaliseExerciseMemory(record?.memory, record?.essentialCue);
      const cue = record?.essentialCue;
      const photoCount = memory.setupPhotos.length;
      ...
      return <article className={`exercise-card ${isExpanded ? 'is-expanded' : 'is-compact'} ${isComplete ? 'is-complete' : ''}`} key={exercise.id} style={{ zIndex: liveSession.exercises.length - index }}>
        <button className="exercise-card-toggle" type="button" aria-expanded={isExpanded} onClick={() => setExpandedExerciseId(exercise.id)}>
          <div><p className="eyebrow">{index + 1} · {exercise.importanceSnapshot}</p><h2>{exercise.exerciseNameSnapshot}</h2></div>
        ...
        {isExpanded && <div className="exercise-card-body">
          ...
          <div className="exercise-support-strip">
            <button type="button" onClick={() => openDetails(exercise.id, 'info')}>Info</button>
            <button type="button" onClick={() => openDetails(exercise.id, 'setup')}>Setup{photoCount > 0 && <span>{photoCount}</span>}</button>
          </div>
          <div className="set-table">{exercise.sets.map((set) => <SetEntryRow key={set.id} set={set} tracking={exercise.trackingSnapshot} bodyweightKg={exercise.bodyweightSnapshotKg} onChange={(patch) => { void updateSetWithUndo(exercise.id, set, patch); }} onCommit={(patch) => commitSetField(exercise.id, set, patch)} onRemove={set.kind === 'additional' ? () => { pendingRestSetIdsRef.current.delete(set.id); void onRemoveSet(liveSession.id, exercise.id, set.id); } : undefined} />)}</div>
          <button className="add-set-button" type="button" onClick={() => { void onAddSet(liveSession.id, exercise.id); }}>＋ Add set</button>
    ...
    <ExerciseDetailsOverlay database={database} exercise={detailsExercise} initialSection={detailsInitialSection} onClose={() => setDetailsExerciseId(null)} onUpdateExercise={onUpdateExercise} />
    ...
    {celebration && <SessionCelebration summary={celebration} onBack={() => setCelebration(null)} onContinue={() => onCompleteSession(liveSession.id)} />}
  </main>;
}
