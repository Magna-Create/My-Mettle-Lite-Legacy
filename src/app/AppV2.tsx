...
import { addSessionSet, amendHistoricalSet, archiveExercise, discardSession, moveRoutineSlot, removeRoutineSlot, removeSessionSet, reorderRoutineSlot, restoreDiscardedSession, setSessionExcluded, updateExerciseRecord, updateRoutineSlot, type ExerciseRecordPatch, type RoutineSlotPatch } from '../application/Phase2Management';
...
import trainIcon from '../assets/icons/sports-martial-arts.svg';
import progressIcon from '../assets/icons/analytics.svg';
import labIcon from '../assets/icons/tactic.svg';
import libraryIcon from '../assets/icons/add-row-below.svg';
...
const tabIcons: Record<Tab, string> = { brief: cycleIcon, train: trainIcon, progress: progressIcon, lab: labIcon, library: libraryIcon };
...
export function AppV2() {
  ...
  return <div className={`app-shell ${restFocused ? 'has-rest-focus' : ''}`}>
    ...
    <div className="page-stack">
      ...
      <section hidden={tab !== 'library'}><LibraryPage database={database} onAddExercise={(input: AddExerciseInput) => run((current) => service.addExerciseToRoutine(current, input))} onReorderSlot={(slotId, direction) => apply((current) => reorderRoutineSlot(current, slotId, direction))} onMoveSlot={(slotId, day) => apply((current) => moveRoutineSlot(current, slotId, day))} onRemoveSlot={(slotId) => apply((current) => removeRoutineSlot(current, slotId))} onUpdateSlot={(slotId, patch: RoutineSlotPatch) => apply((current) => updateRoutineSlot(current, slotId, patch))} onUpdateExercise={(exerciseId, patch: ExerciseRecordPatch) => apply((current) => updateExerciseRecord(current, exerciseId, patch))} onArchiveExercise={(exerciseId) => apply((current) => archiveExercise(current, exerciseId))} onRestoreExercise={(exerciseId) => apply((current) => restoreArchivedExercise(current, exerciseId))} /></section>
    </div>
    <nav className="bottom-nav" aria-label="Primary navigation">{tabs.map((item) => <button key={item} data-active={tab === item} aria-label={tabLabels[item]} title={tabLabels[item]} onClick={() => setTab(item)}><span className="nav-symbol" aria-hidden="true" style={{ '--nav-icon': `url(${tabIcons[item]})` } as CSSProperties} /></button>)}</nav>
