...
export function AppV2() {
  ...
  return <div className={`app-shell ${restFocused ? 'has-rest-focus' : ''}`}>
    ...
    <nav className="bottom-nav" aria-label="Primary navigation">{tabs.map((item) => <button key={item} data-active={tab === item} aria-label={tabLabels[item]} title={tabLabels[item]} onClick={() => setTab(item)}><NavIcon name={item} /></button>)}</nav>
